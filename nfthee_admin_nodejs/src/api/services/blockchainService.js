...
exports.addBlockchain = async (req) => {
    ...
    if (req.file) {
        req.body.icon = `${credentials.BASE_URL}fileUpload/${req.file.filename}`;
    }
...
exports.editBlockchain = async (req) => {
    ...
    const checkName = await blockchainModel.findOne({_id:{ $ne:id },name:req.body.name});
    if(checkName){
        return {
            message: "blockchain name already exists",
            status: false,
    ...
    const blockchain = await blockchainModel.findByIdAndUpdate(id, req.body, {new: true})
    if(blockchain){
        return {
