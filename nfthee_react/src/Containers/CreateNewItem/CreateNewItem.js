...
const CreateNewItem = () => {
  ...
  const [uploadNFT, setUploadNFT] = useState();
  ...
  const handleFileUpload = (event) => {
    ...
    setUploadNFT(event.target.files[0]);
    setTxStatus('');
    setImageView('');
    setMetaDataURl('');
    setTxURL('');
  ...
  const uploadImage = async (event, uploadedFile) => {
  ...
  const uploadNFTContent = async (inputFile) => {
    const nftStorage = new NFTStorage({
      token:
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDU5NDgyNkQxQWRkRDlhMkZCZWRjNDg1NTg2RWFkOUFkN2NlNDhDMmQiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY3MTYwNjAxNDk3MCwibmFtZSI6ImNoZXRhbl9kZXZsbiJ9.5N1_99i4RtjCMCbjGnGYD5V08EUzO8CqGfkdTBtbJKk',
    });
    try {
      ...
      const metaData = await nftStorage.store({
        ...
        external_link: 'test.test',
        seller_fee_basis_points: 100,
        fee_recipient: '0xA97F337c39cccE66adfeCB2BF99C1DdC54C2D721',
      });

      let uri = getIPFSGatewayURL(metaData.url);
      console.log(uri);
